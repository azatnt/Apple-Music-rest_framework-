...
export default class CreateRoomPage extends Component{
    ...
    handleRoomButtonPressed(){
        ...
        fetch('api/create-room', requestOption).then((response) => response.json()).then((data)=>
            ...
            <Grid item xs={12} align="center">
                <Collapse in={this.state.errorMsg != "" || this.state.successMsg != ""}>
                    {this.state.successMsg != "" ?
                        (<Alert severity="success" onClose={() => {this.state({successMsg: ""});
                        }}>{this.state.successMsg}</Alert>)
                        :
                        (<Alert severity="error" onClose={() => {this.state({errorMsg: ""});
